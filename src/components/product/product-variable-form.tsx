        ...
        <div className="mb-5 border-t border-dashed border-border-200 md:mb-8">
          ...
          {inputs.length > 0 && (
            <div className="p-8">
              ...
              <div className="flex justify-between">
                <Input
                  onChange={(e: any) => onHandleChange('forPrice', e)}
                  value={tableInputs.price}
                  name="forPrice"
                  placeholder="price"
                />
                <Input
                  onChange={(e: any) => onHandleChange('forStock', e)}
                  name="forStock"
                  value={tableInputs.stock}
                  className="pl-1"
                  placeholder="Stock"
                />
                <Input
                  onChange={(e: any) => onHandleChange('forSku', e)}
                  name="forSku"
                  value={tableInputs.sku}
                  className="pl-1"
                  placeholder="SKU"
                />
                <Button
                  className="mt-3 ml-1"
                  onClick={(event) => OnApplyClick(event)}
                >
                  On Apply
                </Button>
              </div>
              ...
              <table
                className="mt-5 
            text-left text-sm text-gray-500 dark:text-gray-400"
              >
                <tbody>
                  <th className="bg-gray-100">
                    <tr>
                      {inputs.map((variation, variationIndex) => (
                        <td key={variationIndex} className=" w-28  border p-3 ">
                          {variation.variationName}
                        </td>
                      ))}
                      <td className="w-32  border p-3 ">Price</td>
                      <td className="w-32  border p-3 ">Stock</td>
                      <td className="w-32  border p-3 ">Sku</td>
                    </tr>
                  </th>
                  {inputs.map((variation, variationIndex) => (
                    <>
                      <tr className="mt-3 w-full">
                        {variation.value.map(
                          (input, inputIndex) =>
                            input.value && (
                              <>
                                {/* <td className="border p-5 ">{input.value}</td> */}
                                {variationIndex == 0 && (
                                  <tr>
                                    <td className="w-28 border p-3 text-center ">
                                      {input.value}
                                      {input.image ? (
                                        <img
                                          src={input.image}
                                    ...
                                    <td>
                                      {secondVariationValues.length === 0 ? (
                                        <>
                                          <tr>
                                            <td className="w-32 border p-3">
                                              <Input
                                                value={input.price}
                                                name="price"
                                              />
                                        ...
                                        secondVariationValues.map((r) => {
                                          if (r !== '') {
                                            return (
                                              <tr key={r}>
                                                <td className="w-28 border p-3">
                                                  {r}
                                                </td>
                                                <td className="w-32 border p-3">
                                                  <Input
