...
const FileInput = ({
  ...
  const uploadMultiImageToCloud = async (event: any) => {
    const newFiles: any = Array.from(event.target.files);
    setloading(true);
    newFiles?.map((res: any, index: any) => {
      formData.append('files', res);
    });
    ImagePostFunction('/uploadMultipleUploadFile', formData).then(
      (result1: any) => {
        let imageArr =
          result1 &&
          result1?.map((res: any) => {
  ...
  return (
    <>
      <div className="flex w-full cursor-pointer flex-col border-2 border-dashed border-accent-400">
        <label htmlFor={`${keyPrefix}-file-input`}>
          <div
            className="flex cursor-pointer flex-col items-center justify-center rounded-lg p-5 font-sans md:w-full"
          ...
          >
            ...
            <div className="text-center">
              <p>
                <span className="text-accent-400">Upload an image</span> or drag
                and drop
              </p>
              <p>PNG, JPG</p>
            </div>
